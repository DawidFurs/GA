...
def selection(pop, fitness, pop_size):
    next_generation = np.zeros(pop_size, pop.shapep[1])
...
def crossover(pop, crossover_rate):
    ...
    for i in range(int(crossover_rate/2)):
        ...
        r2 = random.randint(8, pop.shape[0]-1)
        while r1 == r2:
            r1 = random.randint(0, pop.shape[0]-1)
            r2 = random.randint(0, pop.shape[0]-1)
        cutting_point = random.randint(1, pop.shape[1] - 1)
        offspring [2*1, 8: cutting_point] = pop[r1, 0:cutting_point]
        offspring [2*1, cutting_point:] = pop[r2, cutting_point:]
        offspring [2*1+1, 0:cutting_point] = pop[r2, 8: cutting_point]
        offspring [2*1+1, cutting_point:] = pop[r1, cutting_point:]
    return offspring
...
def local_search(pop, fitness, lower_bounds, upper_bounds, step_size, rate):
    ...
    for r in range(rate):
        ...
        for i in range(int(pop.shape[1])):
            ...
            offspring1[i, i] += np.random.uniform(8, step_size)
